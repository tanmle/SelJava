...
public class sanity_007_Exchanging extends AbstractTest{
	...
	@Test(groups = { "regression" },description = "Exchanging Flow")
	public void sanity_Exchanging_001_InventoryFlow()
	{
		log.info("sanity_Exchanging_001 - Step 01: Login with correct username & password");
		schedulePage = loginPage.login(organization, qaStaffUsername, password, false);

		log.info("sanity_Exchanging_001 - Step 01: Go to Patients tab");
		patientsPage = schedulePage.openPatientsPage(driver, ipClient);
		
		validPatientLastName="Udi"+getUniqueNumber();

		log.info("sanity_Exchanging_001 - Step 02: Click New button");
		patientsPage.clickNewPatientButton();
		
		log.info("sanity_Exchanging_001 - Step 03: Input valid last name");
		patientsPage.inputNewPatientLastName(validPatientLastName);
		
		log.info("sanity_Exchanging_001 - Step 04: Input valid home phone");
		patientsPage.inputNewPatientHomePhone(validHomePhone);
		
		log.info("sanity_Exchanging_001 - Step 05: Input valid ZIP");
		patientsPage.inputNewPatientZIP(validZIP);
		
		log.info("sanity_Exchanging_001 - Step 06: Click Next button");
		patientsPage.clickNewPatientNextToStep2Button();
		
		log.info("sanity_Exchanging_001 - Step 07: Input valid first name");
		patientsPage.inputPatientDetailFirstName(validPatientFirstName);
		
		log.info("sanity_Exchanging_001 - Step 08: Input valid patient type");
		patientsPage.selectPatientDetailType(validPatientType);
		
		log.info("sanity_Exchanging_001 - Step 09: Click Save button");
		patientsPage.clickPatientDetailSaveButton();
		
		log.info("sanity_Exchanging_001 - Step 10: Open Inventory page");
		inventoryPage = patientsPage.openInventoryPage(driver, ipClient);
		
		log.info("sanity_Exchanging_001 - Step 11: Select Serialized Item link");
		inventoryPage.clickDynamicInventoryLink("Serialized Items");
		
		log.info("sanity_Exchanging_001 - Step 12: Click New button");
		inventoryPage.clickSerializedItemsNewButton();
		
		log.info("sanity_Exchanging_001 - Step 13: Input new serialized item");
		inventoryPage.inputSerializedItem(office, "Hearing Aid", manufacturer, type, model, serial, "");
		
		log.info("sanity_Exchanging_001 - Step 14: Click Save button");
		inventoryPage.clickSerializedItemsDetailSaveButton();
		
		log.info("sanity_Exchanging_001 - Step 15: Go to Schedule page");
		schedulePage = patientsPage.gotoSchedulePage(driver, ipClient);
		
		log.info("sanity_Exchanging_001 - Step 16: Create new appointment");
		...
		log.info("sanity_Exchanging_001 - Step 17: Open event created");
		schedulePage.openAppointmentOnScheduleDashboard(date, month, year, apptType, validPatientLastName+", "+validPatientFirstName);

		log.info("sanity_Exchanging_001 - Step 18: open Checkin Appointment data");
		patientsPage = schedulePage.clickOnCheckinAppointmentDataButton();
		
		log.info("sanity_Exchanging_001 - step 19: Open Create sales order page");
		salesPage =  patientsPage.clickOnCreateSalesOrder();
		
		log.info("sanity_Exchanging_001 - step 20: Add item");
		salesPage.addItemInCreateSalesOrder("Add Hearing Aid");

		log.info("sanity_Exchanging_001 - step 21: Enter Left Ear");
		salesPage.enterHearingAidDetailsLeftEar(manufacturer, type, model,serial,battery);

		log.info("sanity_Exchanging_001 - step 22: Enter Left Ear Discount Type");
		salesPage.selectPrincingDiscountType("Monetary");

		log.info("sanity_Exchanging_001 - step 23: Enter Left Ear Pricing Discount");
		salesPage.enterPrincingDiscount("750");
		
		log.info("sanity_Exchanging_001 - step 24: click Save");
		salesPage.clickOnSaveButton();

		log.info("sanity_Exchanging_001 - Step 25: Click Back button");
		salesPage.clickOnBackButton();
		
		log.info("sanity_Exchanging_001 - Step 26: Click Checkout button");
		salesPage.clickOnCheckoutButton();
		
		log.info("sanity_Exchanging_001 - Step 27: Click Exchange button");
		salesPage.clickOnExchangeButton();
	
		log.info("sanity_Exchanging_001 - Step 28: Select return to option");
		salesPage.selectReturnToOption("Inventory");
		
		log.info("sanity_Exchanging_001 - Step 29: Click Save and Select New button");
		salesPage.clickSaveAndSelectNewButton();
